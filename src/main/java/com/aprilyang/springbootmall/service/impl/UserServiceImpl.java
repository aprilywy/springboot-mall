...
public class UserServiceImpl implements UserService {
    ...
    public Integer register(UserRegisterRequest userRegisterRequest) {
        ...
        // 檢查註冊的 userName
        User useru = userDao.getUserByUserName(userRegisterRequest.getUsername());

        if (useru.getUsername() != null){
            log.warn("該 帳號 {} 已經被註冊", userRegisterRequest.getUsername());
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST);
        }
    ...
    public User login(UserLoginRequest userLoginRequest) {
        ...
        // 檢查 user 是否存在
        if (user == null) {
            log.warn("該 帳號 {} 尚未註冊", userLoginRequest.getUsername());
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST);
        }
        ...
        if (user.getPassword().equals(hashedPassword)) {
            ...
            log.warn("此帳號 {} 的密碼不正確", userLoginRequest.getUsername());
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST);
        }
    }
}
